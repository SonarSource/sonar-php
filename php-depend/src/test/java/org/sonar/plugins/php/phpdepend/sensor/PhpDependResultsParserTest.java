...
public class PhpDependResultsParserTest {
	...
	private void init() {
		try {
			File xmlReport = new File(getClass().getResource(
					"/org/sonar/plugins/php/phpdepend/sensor/PhpDependResultsParserTest/pdepend.xml").toURI());
	...
	public void shouldThrowAnExceptionWhenReportIsInvalid() {
		project = mock(Project.class);
		PhpDependResultsParser parser = new PhpDependResultsParser(project, null);
		parser.parse(new File(getClass().getResource(
				"/org/sonar/plugins/php/phpdepend/sensor/PhpDependResultsParserTest/pdepend-invalid.xml").getFile()));
	}
	...
	public void shouldNotGenerateDirOrProjectMeasures() {
		...
		verify(context, never()).saveMeasure(eq(new org.sonar.api.resources.Directory("Sources/main/Common")), eq(metric),
				anyDouble());
	}
