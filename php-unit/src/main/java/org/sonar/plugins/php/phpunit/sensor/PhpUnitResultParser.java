...
public class PhpUnitResultParser {
	...
	private TestSuites getTestSuites(File report) {
		...
		try {
			...
			return (TestSuites) xstream.fromXML(inputStream);
		} catch (IOException e) {
	...
	public List<PhpUnitTestReport> readSuites(TestSuites testSuites) {
		...
		for (TestSuite testSuite : testSuites.getTestSuites()) {
			result.addAll(new PhpTestSuiteReader().readSuite(testSuite));
		}
		return result;
	}
	 ...
	 * @param project the project
	 */
	private void saveClassMeasure(SensorContext context, PhpUnitTestReport fileReport, Metric metric, double value,
	    Project project) {
	...
	/**
	 * Saves the measures contains in the test report.
	 * 
	 * @param context the execution context
	 * @param project the analyzed project
	...
	private void saveTestsDetails(SensorContext context, PhpUnitTestReport fileReport, Project project) {
		...
		for (TestCase detail : fileReport.getDetails()) {
			...
			if (isError || TestCase.STATUS_FAILURE.equals(detail.getStatus())) {
				testCaseDetails.append(">")
				    ...
				    .append("\">")
				    .append("<![CDATA[")
				    .append(StringEscapeUtils.escapeXml(detail.getStackTrace()))
				    .append("]]>")
		...
		testCaseDetails.append("</tests-details>");
		context.saveMeasure(getUnitTestResource(fileReport, project), new Measure(CoreMetrics.TEST_DATA,
