...
public class CookiesSecureCheck extends PHPVisitorCheck implements PhpIniCheck {
  ...
  public void visitFunctionCall(FunctionCallTree tree) {
    ...
    if (SET_COOKIE_FUNCTIONS.contains(functionName)) {
      NewIssueIfBadFlag(tree, SET_COOKIE_SECURE_PARAMETER);
    }
    else if (SESSION_COOKIE_FUNC.equals(functionName)) {
      NewIssueIfBadFlag(tree, SESSION_COOKIE_SECURE_PARAMETER);
    }

    super.visitFunctionCall(tree);
  }

  private void NewIssueIfBadFlag(FunctionCallTree tree, int argumentIndex) {
