...
public class SSLCertificatesVerificationDisabledCheck extends PHPVisitorCheck {
  ...
  private static final Set<String> verifyHostCompliantValues = ImmutableSet.of("2");
  private static final Set<String> verifyPeerCompliantValues = ImmutableSet.of("true", "1");
  ...
  private void checkCURLSSLVerify(ExpressionTree expressionTree, Set<String> compliantValues) {
    ...
    if (curlOptValue instanceof LiteralTree) {
      ...
      String quoteLessLowercaseValue = CheckUtils.trimQuotes(value).toLowerCase();
      isCompliant = compliantValues.contains(quoteLessLowercaseValue);
    }
