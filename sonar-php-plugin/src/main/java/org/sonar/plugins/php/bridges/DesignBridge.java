...
public class DesignBridge extends Bridge {
  ...
  public void onProject(SourceProject squidProject, Project sonarProject) {
    Set<SourceCode> squidPackages = getSourcePackages(squidProject);
  ...
  private Set<SourceCode> getSourcePackages(SourceProject squidProject) {
    ...
    for (SourceCode squidPackage : squidProject.getChildren()) {
      ...
      squidPackages.add(squidPackage);
    }
    return squidPackages;
  }
  ...
  public void onPackage(SourcePackage squidPackage, Resource sonarPackage) {
    Set<SourceCode> squidFiles = getSourceFiles(squidPackage);
    LOG.debug("package: " + squidPackage.getKey());
    LOG.debug("{} files to analyze", squidFiles.size());
    if (!squidFiles.isEmpty()) {
  ...
  private Set<SourceCode> getSourceFiles(SourcePackage squidPackage) {
    Set<SourceCode> squidFiles = new TreeSet<SourceCode>();
    for (SourceCode squidFile : squidPackage.getChildren()) {
      if (!SourceFile.class.isInstance(squidFile)) {
        continue;
      }
      squidFiles.add(squidFile);
    }
    return squidFiles;
  }
  ...
  private String serializeDsm(PHPSquid squid, Set<SourceCode> squidSources, Set<Edge> feedbackEdges) {
  ...
  private Dependency saveEdge(SourceCodeEdge edge, SensorContext context, Dependency parentDependency) {
    ...
    if (dependency == null) {
      ...
      if (from != null && to != null) {
        ...
        if (from == null) {
          LOG.warn("Unable to find from resource '" + edge.getFrom() + "' to create a dependency with '" + edge.getTo() + "'");
        }
        if (to == null) {
          LOG.warn("Unable to find to resource '" + edge.getTo() + "' to create a dependency with '" + edge.getFrom() + "'");
        }
        return null;
      }
    }
