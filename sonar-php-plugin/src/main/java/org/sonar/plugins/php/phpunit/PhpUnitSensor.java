...
@Properties({
  ...
  @Property(key = PHPUNIT_MAIN_TEST_FILE_KEY, defaultValue = "",
    name = "[DEPRECATED] File containing the main method calling all the tests", project = false, global = false,
  ...
  @Property(key = PHPUNIT_ANALYZE_TEST_DIRECTORY_KEY, defaultValue = PHPUNIT_ANALYZE_TEST_DIRECTORY_DEFVALUE,
    name = "[DEPRECATED] Should analyse the whole test directory", project = false, global = false,
  ...
  @Property(key = PHPUNIT_IGNORE_CONFIGURATION_KEY, defaultValue = "false", name = "[DEPRECATED] Ignore default configuration", project = false,
    global = false, description = "[DEPRECATED] Ignore default configuration file (phpunit.xml).", category = PhpUnitSensor.CATEGORY_PHP_PHP_UNIT,
    type = PropertyType.BOOLEAN),
