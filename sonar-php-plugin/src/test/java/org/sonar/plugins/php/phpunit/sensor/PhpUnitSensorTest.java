...
public class PhpUnitSensorTest {
  ...
  public void shouldNotLaunchOnNonPhpProject() {
    ...
    when(configuration.getString(PhpUnitConfiguration.COVERAGE_REPORT_FILE_PROPERTY_KEY, PhpUnitConfiguration.DEFAULT_REPORT_FILE_NAME))
        .thenReturn("punit.summary.xml");
    when(
  ...
  public void shouldNotLaunchWhenConfiguredSoOnPhpProject() {
    ...
    when(project.getLanguage()).thenReturn(Php.INSTANCE);
    when(
        configuration.getBoolean(PhpUnitConfiguration.SHOULD_RUN_PROPERTY_KEY, Boolean.getBoolean(PhpUnitConfiguration.DEFAULT_SHOULD_RUN)))
        .thenReturn(false);
    ProjectFileSystem fs = mock(ProjectFileSystem.class);
    when(project.getFileSystem()).thenReturn(fs);
    when(fs.getBuildDir()).thenReturn(new File(PhpUnitConfiguration.DEFAULT_REPORT_FILE_PATH));
  ...
  private void init() {
    ...
    when(
        configuration.getString(PhpUnitConfiguration.REPORT_FILE_RELATIVE_PATH_PROPERTY_KEY, PhpUnitConfiguration.DEFAULT_REPORT_FILE_PATH))
        .thenReturn("/log");
    when(configuration.getString(PhpUnitConfiguration.COVERAGE_REPORT_FILE_PROPERTY_KEY, PhpUnitConfiguration.DEFAULT_COVERAGE_REPORT_FILE))
        .thenReturn("phpunit.coverage.xml");
    when(
        configuration.getBoolean(PhpUnitConfiguration.SHOULD_RUN_PROPERTY_KEY, Boolean.getBoolean(PhpUnitConfiguration.DEFAULT_SHOULD_RUN)))
        .thenReturn(true);
    when(project.getConfiguration()).thenReturn(configuration);
