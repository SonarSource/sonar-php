...
public class PhpDependResultsParserTest {
	...
	private void init(String pdependResultFile) {
		try {
			...
			when(fileSystem.getSourceDirs()).thenReturn(
					Arrays.asList(new File(
							"C:\\projets\\PHP\\Money\\Sources\\main")));
			when(fileSystem.getTestDirs()).thenReturn(
					Arrays.asList(new File(
							"C:\\projets\\PHP\\Money\\Sources\\test")));
			Set<Metric> metrics = new HashSet<Metric>();
			metrics.add(metric);
			PhpDependResultsParser parser = new PhpDependResultsParser(project,
					context, metrics);
	...
	public void shouldThrowAnExceptionWhenReportIsInvalid() {
		...
		parser.parse(new File(
				getClass()
						.getResource(
								"/org/sonar/plugins/php/phpdepend/sensor/parser/pdepend-invalid.xml")
						.getFile()));
	}
	...
	public void shouldGenerateLocMeasures() {
		...
		init(PDEPEND_RESULT);
		verify(context).saveMeasure(new PhpFile("MoneyTest.php", true), metric,
				255.0);
		...
		verify(context).saveMeasure(new PhpFile("Common/IMoney.php"), metric,
				74.0);
	}
	...
	public void shouldGenerateNclocMeasures() {
		...
		verify(context)
				.saveMeasure(new PhpFile("MoneyTest.php"), metric, 152.0);
		verify(context).saveMeasure(new PhpFile("Common/IMoney.php"), metric,
				15.0);
	}
	...
	public void shouldNotGenerateDirOrProjectMeasures() {
		...
		verify(context, never()).saveMeasure(
				eq(new org.sonar.api.resources.Directory("Sources/main")),
				eq(metric), anyDouble());
		verify(context, never())
				.saveMeasure(
						eq(new org.sonar.api.resources.Directory(
								"Sources/main/Common")), eq(metric),
						anyDouble());
	}
