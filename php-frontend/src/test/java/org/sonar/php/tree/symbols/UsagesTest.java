...
public class UsagesTest extends ParsingTestUtils {
  ...
  private void test_class(Scope scope) {
    assertThat(scope.getSymbol("$field").usages()).hasSize(0); // fixme : should be 1
    assertThat(scope.getSymbol("method").usages()).hasSize(0); // fixme : should be 1
  }
  ...
  private void test_use_clause(Scope scope) {
    ...
    assertThat(useClauseSymbolB).isEqualTo(globalSymbolB);
